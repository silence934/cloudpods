...
func TestSecurityGroupRulesCompare0(t *testing.T) {
	...
	for i := 0; i < length; i++ {
		...
		rule.PortStart = rand.Intn(rule.PortEnd - 5)
...
func TestSecurityGroupRulesCompare1(t *testing.T) {
	...
	dest.Rules[index].PortStart++

	common, inAdds, outAdds, inDels, outDels := cloudprovider.CompareRules(src, dest, false)

	if common.Len() == length-1 {
		if dest.Rules[index].Direction == secrules.SecurityRuleIngress {
			if len(outDels) == 0 && len(outAdds) == 0 && len(inAdds) == 1 && len(inDels) == 1 {
				if inAdds[0].String() == src.Rules[index].String() && inDels[0].String() == dest.Rules[index].String() {
					return
				}
			}
		} else {
			if len(outDels) == 1 && len(outAdds) == 1 && len(inAdds) == 0 && len(inDels) == 0 {
				if outAdds[0].String() == src.Rules[index].String() && outDels[0].String() == dest.Rules[index].String() {
					return
				}
			}
		}
...
func TestSecurityGroupRulesCompare2(t *testing.T) {
